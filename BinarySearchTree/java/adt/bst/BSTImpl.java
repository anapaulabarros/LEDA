...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	private int heightTree(BSTNode<T> node) {
		if(node.isEmpty())
			return -1;
		else
			return( 1 + Math.max(heightTree((BSTNode<T>) node.getLeft()), heightTree((BSTNode<T>) node.getRight())));
	}
	...
	public BSTNode<T> sucessor(T element) {
		...
		if(node.isEmpty())
			...
			return sucessorTree(node);
	}

	private BSTNode<T> sucessorTree(BSTNode<T> node) {
		if(!node.getRight().isEmpty())
			return minimumTree((BSTNode<T>) node.getRight());
		else{
			BSTNode<T> aux = (BSTNode<T>)node.getParent();
			while (!aux.isEmpty() && node.equals(aux.getRight())) {
				node = aux;
	...
	public BSTNode<T> predecessor(T element) {
		...
		if(node.isEmpty())
			...
			return predecessorTree(node);
	}

	private BSTNode<T> predecessorTree(BSTNode<T> node) {
		if(!node.getLeft().isEmpty())
			return maximumTree((BSTNode<T>)node.getLeft());
		else{
			BSTNode<T> aux = (BSTNode<T>)node.getParent();
			while (!aux.isEmpty() && node.equals(aux.getLeft())) {
				node = aux;
				aux = (BSTNode<T>) aux.getParent();
			}
			return aux;
	...
	private BSTNode<T> remove(T element, BSTNode<T> node) {
		if(node.isEmpty())
